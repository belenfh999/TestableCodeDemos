...
class PrintHardTest < Minitest::Test
  ...
  def setup
    username = 'Arthur'
    @identity = IdentityService.new(username)
    @identity.stubs(:get_username).returns(username)
  ...
  def test_print_inv1
    ...
    @database.stubs(:puts)
    printer = @print_command.invoice_writer.printer
    ...
    assert_equal(expected_username, @print_command.identity.get_username)
  ...
  def test_print_inv2
    ...
    @identity = IdentityService.new("Eva")
  ...
  def set_print_invoice_command(color)
    ...
    @print_command = Module4::PrintEasyInvoiceCommand.new(@database, Module4::InvoiceWriter.new(printer, page_layout, @stubbed_date), @identity)
  end
